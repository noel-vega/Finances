...
flowchart TB
    subgraph FE["Frontends"]
        ...
        adminweb["admin-web<br/>React 19 + Vite · :5000<br/><i>merchant dashboard</i>"]
    ...
    subgraph API["APIs"]
        adminapi["admin-api<br/>NestJS/Fastify · :3000<br/><i>products, inventory, orders,<br/>staff, Stripe Connect</i>"]
...
- No backend-to-backend HTTP traffic — `admin-api` and `storefront-api` never call each
  other. The only inter-service calls are frontend → its own API, via a generated SDK.
- All three backend services (`admin-api`, `storefront-api`, `worker`) expose an unauthenticated
...
- `admin-web`'s `vite.config.ts` says port 3001, but its `dev` script passes `--port 5000`,
...
- Every account is a tenant with its own Stripe Connect account — `admin-api` and
...
(no `orders` queue involved): `staff-invite` from `admin-api` and `customer-thank-you`
