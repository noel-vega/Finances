...
#   docker build -f docker/merchant-api.Dockerfile -t ordersail-merchant-api:local .
...
FROM deps AS build
COPY . .
RUN npm run build --workspace=config --workspace=logging --workspace=db --workspace=queue --workspace=storage
...
# every workspace (not just this app's), which pulls in some unused weight;
...
CMD ["node", "apps/merchant-api/dist/src/main.js"]
