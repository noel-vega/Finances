...
variable "name" {
  description = "App short name, e.g. \"admin-api\" — used in resource names/tags."
  type        = string
}
...
variable "task_role_policy_json" {
  description = "Optional extra IAM policy JSON attached to the task role (e.g. admin-api's S3 access to the product-images bucket)."
  type        = string
  default     = null
}
