...
# admin-api's task role: direct S3 access to the product-images
...
module "ecs_service_admin_api" {
  ...
  ecs_tasks_security_group_id = module.ecs_cluster.ecs_tasks_security_group_id
  container_port              = 3000
  image                       = "${module.ecr.repository_urls["admin-api"]}:latest"
  target_group_arn            = module.alb_admin_api.target_group_arn
  task_role_policy_json       = data.aws_iam_policy_document.admin_api_task.json
  ...
  environment = [
    ...
    # references the module directly, not local.frontends["admin-web"] — that local
    ...
    { name = "SHOP_ADMIN_WEB_URL", value = "https://${module.frontend_admin_web.distribution_domain_name}" },
  ...
  secrets = [
    ...
    { name = "STAFF_JWT_SECRET", valueFrom = "${module.secrets.app_secret_arns["admin-api"]}:STAFF_JWT_SECRET::" },
    { name = "STRIPE_SECRET_KEY", valueFrom = "${module.secrets.app_secret_arns["admin-api"]}:STRIPE_SECRET_KEY::" },
    { name = "STRIPE_ACCOUNT_WEBHOOK_SECRET", valueFrom = "${module.secrets.app_secret_arns["admin-api"]}:STRIPE_ACCOUNT_WEBHOOK_SECRET::" },
    { name = "SHIPPO_API_KEY", valueFrom = "${module.secrets.app_secret_arns["admin-api"]}:SHIPPO_API_KEY::" },
  ]

  secrets_manager_secret_arns = [
    module.secrets.database_url_secret_arn,
    module.secrets.app_secret_arns["admin-api"],
  ]
}
...
module "ecs_service_storefront_api" {
  ...
  environment = [
    ...
    { name = "REDIS_HOST", value = module.elasticache.primary_endpoint_address },
    { name = "REDIS_PORT", value = tostring(module.elasticache.port) },
    # see the admin-api service's SHOP_ADMIN_WEB_URL comment above — same reason to
...
module "frontend_admin_web" {
  ...
  api_origin_domain_name = module.alb_admin_api.dns_name
}
