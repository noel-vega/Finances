...
locals {
  ...
  ssm_frontend_admin_web = {
    "frontend/admin-web-bucket"       = module.frontend_admin_web.bucket_name
    "frontend/admin-web-distribution" = module.frontend_admin_web.distribution_id
  }
  ...
  ssm_ecr = {
    "ecr/admin-api-uri" = module.ecr.repository_urls["admin-api"]
  ...
  ssm_ecs = {
    ...
    "ecs/admin-api-service" = module.ecs_service_admin_api.service_name
  ...
  ssm_alb = {
    "alb/admin-api-dns" = module.alb_admin_api.dns_name
...
resource "aws_ssm_parameter" "frontend_admin_web" {
  for_each = local.ssm_frontend_admin_web
  name     = "/${var.name_prefix}/production/${each.key}"
  type     = "String"
  value    = each.value
}
